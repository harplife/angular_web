webapp.controller('TargetEditModalCtrl', [ '$window', '$scope', '$uibModalInstance', 'toModalData', 'GLOBAL_CONSTANT', 'ModalAPISrv', 'postApi', 'gAlert', '$rootScope', 'modalFactory', 'gConfirm', '$filter', '$translate',
  function TargetEditModalCtrl($window, $scope, $uibModalInstance, toModalData, CONSTANT, modalApi, postApi, gAlert, $rootScope, modalFactory, gConfirm, $filter, $translate) {

    var vm = this;
    vm.dataloader = false;

    // SQL Mapper Id 정의
    var INSERT_SQL = 'WjmSurvey.insertSvyTarget';     // 등록 SQL
    var UPDATE_SQL = 'WjmSurvey.updateSvyTarget';     // 수정 SQL
    var VIEW_SQL = 'WjmSurvey.getSvyTargetDetail'; // 조회 SQL
    var DUPCHK_SQL = 'WjmSurvey.checkSvyTarget';         // 중복체크 SQL
    ...
    function getIsDup(type) {
      var data = {};
      switch(type){
        case "M": data = {SvyId: vm.data.SvyId, Mobile: vm.data.Mobile}; break;
        case "E": data = {SvyId: vm.data.SvyId, Email: vm.data.Email}; break;
      }
