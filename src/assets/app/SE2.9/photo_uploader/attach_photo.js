  ...
  var aResultleng = 0;
	...
	//마크업-공통
  var welBtnFind = $Element("btn_find");        //확인 버튼
    ...
    function html5Upload() {
      ...
      var userkey = window.sessionStorage['userkey'] + '-' + window.sessionStorage['loginTenantId'];

//    	var config=JSON.parse(global_config);

    	var sUploadURL= API_HTTP_PROTOCOL + "//" + API_SERVER_DOMAIN + ":" + API_SERVER_PORT + '/file/imageupload.do?userkey=' + userkey; 	//upload URL
...
//	    			callAjax(tempFile, sUploadURL);
    ...
    function callAjax(files, uploadUrl){
...
/*     function callAjaxForHTML5_new(files, data) {
			var xhr = new XMLHttpRequest();
			requests.push(xhr);
			return {
...
//      xhr.upload.onprogress = onProgress;
      xhr.onload = function(e) {
        ...
        if (xhr.readyState == 4) {
          if (sResString.indexOf("NOTALLOW_") > -1) {
            ...
            makeArrayFromString(xhr.response);
          }
        }
      };
    ...
    function makeArrayFromString(sResString) {
      ...
      try {
        ...
        for (var i = 0; i < aTemp.length; i++) {
          if (!!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0) {
            ...
            if (aSubTemp[0] == "sFileURL") {
              htTemp[aSubTemp[0]] = API_HTTP_PROTOCOL + "//" + API_SERVER_DOMAIN + ":" + API_SERVER_PORT + aSubTemp[1];
            } else {
      ...
      console.log(aResultleng, nImageFileCount);
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/jsp/file_uploader.jsp',	//샘플 URL입니다.
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
    ...
    function filechanged() {
      ...
      if (!rFilter.test(file.type)) {
        alert("이미지파일 (jpg,gif,png)만 업로드 가능합니다.");
      } else if(file.size > nMaxImageSize){
        alert("이미지 용량이 10MB를 초과하여 등록할 수 없습니다.");
      } else {
        //제한된 수만 업로드 가능.
        if ( j < nMaxImageCount ){
          sListTag += addImage(file);
	...
	window.onload = function(){
      ...
      $Fn(findFile,this).attach(welBtnFind.$value(), "click");
	};
